...
"""
    @TODO:
        Dosent support ortho view rendering
        See how to add support for this in 
        archipack_manipulators Gl.draw()
"""
...
class ARCHIPACK_OT_render(Operator):
    ...
    def execute(self, context):
        ...
        # Use default render
        # -----------------------------
        # if scene.archipack_render_type == "1":
        # noinspection PyBroadException
        try:
            result = bpy.data.images['Render Result']
            if result.has_data is False:
        ...
        if render_main(self, context, objlist) is True:
        ...
        # OpenGL image
        # -----------------------------
        if scene.archipack_render_type == "2":
        ...
        # OpenGL Animation
        # -----------------------------
        if scene.archipack_render_type == "3":
        ...
        # Image
        # -----------------------------
        if scene.archipack_render_type == "4":
        ...
        # Animation
        # -----------------------------
        if scene.archipack_render_type == "5":
            oldframe = scene.frame_current
            flag = False
            # loop frames
            for frm in range(scene.frame_start, scene.frame_end + 1):
