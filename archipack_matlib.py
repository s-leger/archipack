...
class MatLib():
    """
        Represents a material library .blend file or scene
        Load materials into file with fake user set
        Build a icon enumerator
    """
    def __init__(self):
        self.source = 'NONE'
        self.ready = False
        # absolute path of library file
        self.folder = None
        # library filename without .blend
        self.filename = None
        self.enum = []
        self.libs_enum = []
    ...
    def has(self, name):
        return name in [m[0] for m in self.enum]
    
    def make_enum(self, mats):
        # Build enum using native material's Icon
        # Pros: icon is native and always up to date
        # Cons: material must be loaded into scene        
        mats.sort(key=lambda mat: mat.name.lower())
        self.enum.clear()
        self.enum.extend([
                    (mat.name, 
                    mat.name.capitalize(), 
    ...
    def get_enum(self, context):
        # dynamically refresh enum
        # for all but library file
    ...
    def library(self, context):
        """
            ...
            With icons thumbs previews
        """
        self.reset()
        self.source = 'LIBRARY'
        filename = "{}.blend".format(os.path.join(self.folder, self.filename))
        if os.path.isfile(filename):
            try:
                with bpy.data.libraries.load(filename, False, False) as (data_from, data_to):
    ...
    def library_enums(self):
        libs = [f[:-6] for f in os.listdir(self.folder) if f[-5::] == "blend"]
        libs.sort(key=lambda x: bpy.path.display_name(x))  
        self.libs_enum.clear()
        self.libs_enum.extend([
            (lib,  
            bpy.path.display_name(lib), 
            bpy.path.display_name(lib) 
            )
    ...
    def reset(self):
        if self.source == 'LIBRARY':
            names = [m[0] for m in self.enum]
            for name in names:
                # cleanup unused materials datablocks        
    ...
    def save_material(self, name):
        """
          Create and executre save material script
        """
...
def mat_items(self, context):
    ...
    if matlib is None:
        matlib = MatLib()
    return matlib.get_enum(context)

    
def lib_items(self, context):
    global matlib
    if matlib is None:
        matlib = MatLib()
    return matlib.libs_enum
...
class archipack_matlib(PropertyGroup):
    ...
    def update(self, context):
        ...
        if self.source == 'LIBRARY':
            if self.folder != matlib.folder:
                matlib.folder = self.folder
                matlib.library_enums()
                reload = True
            if self.filename != matlib.filename:
                matlib.filename = self.filename
                reload = True
                
        if self.source != matlib.source:
            reload = True
        
        if reload:
            if self.source == 'LIBRARY':
                matlib.library(context)
            elif self.source == 'SCENE':
                matlib.scene(context)
            elif self.source == 'SELECTED':
                matlib.selected(context)    
            elif self.source == 'ACTIVE':
                matlib.active(context)
    ...
    def update_mat(self, context):
        ...
        if mat and o and o.active_material:
            mat.use_fake_user = False
            o.active_material = mat
        # context.area.tag_redraw()
        
        
class ARCHIPACK_PT_matlib(Panel):
    bl_idname = "ARCHIPACK_PT_matlib"
    bl_label = "Archipack Matlib"
    ...
    def draw(self, context):
        layout = self.layout
        d = context.scene.archipack_matlib
        scale = int(d.scale)
        layout.template_icon_view(d, "mat", show_labels=scale > 2, scale=scale)
        layout.prop(d, 'folder')
        layout.prop(d, 'filename')
        layout.prop(d, 'scale', expand=True)
        layout.prop(d, 'source', expand=True)
        layout.operator('archipack.matlib_reload')
        # row.operator('archipack.material_add', icon="ZOOMIN", text="")
        # row.operator('archipack.material_remove', icon="ZOOMOUT", text="")


class ARCHIPACK_OT_matlib_reload(Operator):
    bl_idname = "archipack.matlib_reload"
    bl_label = "Reload"
    bl_description = "Reload a material library"
    bl_options = {'REGISTER', 'UNDO'}

    @classmethod
    def poll(cls, context):
        global matlib
        return matlib is not None

    def execute(self, context):
        global matlib
        matlib.reload(context)
        return {'FINISHED'}


            
"""
from archipack import archipack_matlib
archipack_matlib.register()

archipack_matlib.unregister()
...
def register():
    ...
    Scene.archipack_matlib = PointerProperty(type=archipack_matlib)
    bpy.utils.register_class(ARCHIPACK_OT_matlib_reload)
...
def unregister():
    ...
    bpy.utils.unregister_class(ARCHIPACK_OT_matlib_reload)
    del Scene.archipack_matlib
