...
class DoorProperty(Manipulable, PropertyGroup):
    ...
    def verts(self):
        # door inner space 
        v = Vector((0,0,0))
        size = Vector((self.x, self.z, self.y))                             #offset, center, origin, size, radius, 0, pivot,
    ...
    def create_childs(self, context, o):
        ...
        if n_childs < 1:
            ...
            bpy.ops.archipack.door_panel(x=self.x, z=self.z, door_y=self.door_y, n_panels=self.n_panels, direction=self.direction)
        ...
        if self.n_panels == 2 and n_childs < 2:
            ...
            bpy.ops.archipack.door_panel(x=self.x, z=self.z, door_y=self.door_y, n_panels=self.n_panels, direction=1-self.direction)
            child = context.object
            child.parent = o
            child.matrix_world = o.matrix_world.copy()
            location = self.x/2 + BATTUE - SPACING
    ...
    def interactive_hole(self, context, o):
        ...
        size = Vector((self.x+2*self.frame_x, self.z+self.frame_x+0.001, self.y))                             #offset, center, origin, size, radius, 0, pivot,
