...
def generateThumb(context, cls, preset):
    ...
    if cls == 'archipack_curves':
        ...
        context.scene.world.horizon_color = (1, 1, 1)

        from archipack.archipack_curveman import ArchipackProfileManager
        man = ArchipackProfileManager()
        ...
        m.use_nodes = True
        m.node_tree.nodes[1].inputs[0].default_value = (0, 0, 0, 1)
        curve.data.materials.append(m)

        bpy.ops.object.camera_add(view_align=True,
            enter_editmode=False,
            location=(0.5 * x, 0.5 * y, 1),
            rotation=(0, 0, 0))
