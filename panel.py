...
class Panel():
    ...
    def _get_trapezoidal_coords(self, offset, origin, size, angle_y, x, pivot, bottom_y=0):
        """
            Rectangular trapeze vertical   
            ...
            TODO:
            use center and radius as basis
            so it is able to work the same way as triangle and pentagons do
        """
        coords = []
        ta = tan(angle_y)
        dy = offset.y + size.y + x/cos(angle_y) - ta * (origin.x+size.x)
        x_left =  offset.x + size.x/2 * (pivot-1) + x
        x_right = offset.x + size.x/2 * (pivot+1) - x
        # bottom left
        if self.closed_path:
        ...
        # top left
        coords.append((x_left, dy + ta * (origin.x + x_left)))
        # top right
        coords.append((x_right, dy + ta * (origin.x + x_right)))
    ...
    def _get_horizontal_rectangular_trapezoid_coords(self, offset, center, origin, size, basis, x, pivot, bottom_y=0):
        ...
        """
        coords = []
        # find wich side of tri we are on
        cx = (0.5*basis+center.x) - origin.x
        x_left =  size.x/2 * (pivot-1) + x
        x_right = size.x/2 * (pivot+1) - x
        y0 = self.intersect_triangle(cx, cy, basis, x_left - x)
        y1 = self.intersect_triangle(cx, cy, basis, x_right + x)
        
        if y0 <= origin.y:
            # left size
            ta = center.y/(0.5*basis+center.x)
        else:
            # right size
            ta = center.y/(0.5*basis-center.x)
        
        angle_y = atan(ta)
        dy = offset.y + size.y - x/cos(angle_y)
        x_left =  size.x/2 * (pivot-1) + x
        x_right = size.x/2 * (pivot+1) - x
        # bottom left
        if self.closed_path:
    ...
    def _get_pentagon_coords(self, offset, center, origin, size, basis, x, pivot, bottom_y=0):
        ...
        if self.closed_path:
            ...
            coords.append((x_left, offset.y + bottom_y))
        # top left
        coords.append((x_left, offset.y + y0))
        # top center
        coords.append((center.x, offset.y + center.y))
        # top right
        coords.append((x_right, offset.y + y1))
        # bottom right
        if self.closed_path:
            coords.append((x_right, offset.y + x + bottom_y))
        else:
    ...
    def vertices(self, steps, offset, center, origin, size, radius, angle_y, pivot, shape_z=None, path_type='ROUND', axis='XZ'):
        ...
        if path_type == 'ROUND':
            coords = [self._get_arc_coords(steps, offset, center, origin, size, Vector((radius.x-x,0)), x, pivot, shape_z[i]) for i, x in enumerate(self.x)]
        elif path_type == 'ELLIPSIS':
            coords = [self._get_ellispe_coords(steps, offset, center, origin, size, Vector((radius.x-x, radius.y-x)), x, pivot, shape_z[i]) for i, x in enumerate(self.x)]
        elif path_type == 'QUADRI':
            coords = [self._get_vertical_rectangular_trapezoid_coords(offset, center, origin, size, radius.x, x, pivot) for i, x in enumerate(self.x)]
            #coords = [self._get_trapezoidal_coords(offset, origin, size, angle_y, x, pivot, shape_z[i]) for i, x in enumerate(self.x)]
        elif path_type == 'HORIZONTAL':
            coords = [self._get_horizontal_coords(offset, size, x, pivot) for i, x in enumerate(self.x)]
        elif path_type == 'VERTICAL':
            coords = [self._get_vertical_coords(offset, size, x, pivot) for i, x in enumerate(self.x)]
        elif path_type == 'CIRCLE':
    ...
    def uv(self, steps, center, origin, size, radius, angle_y, pivot, x, x_cap, path_type='ROUND'):
        ...
        if self.side_cap_back > -1 or self.side_cap_front > -1:
            if path_type == 'ROUND':
                ...
                coords = self._get_arc_coords(steps, Vector((0,0,0)), center, origin, size, Vector((radius.x-x_cap,0)), x_cap, pivot, x_cap)
            elif path_type == 'CIRCLE':
                # full circle
                coords = self._get_circle_coords(steps, Vector((0,0,0)), center, origin, Vector((radius.x-x_cap,0)))
            elif path_type == 'ELLIPSIS':
                coords = self._get_ellispe_coords(steps, Vector((0,0,0)), center, origin, size, Vector((radius.x-x_cap, radius.y-x_cap)), x_cap, pivot, x_cap)
            elif path_type == 'QUADRI':
                coords = self._get_vertical_rectangular_trapezoid_coords(Vector((0,0,0)), center, origin, size, radius.x, x_cap, pivot)
