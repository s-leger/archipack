...
class PrecisionModel():
    ...
    def __init__(self, nModelType=None):

        if nModelType is None:
            nModelType = PrecisionModel.FLOATING

        self.modelType = nModelType
        self.scale = 0.0
        self.maximumPreciseValue = 9007199254740992.0

    def makePrecise(self, coord):
        pass
...
class GeometryTransformer():
    ...
    def transform(self, geom):
        ...
        elif geom.geometryTypeId == GeometryTypeId.GEOS_LINESTRING:
            return self.transformLineString(geom, None)
        elif geom.geometryTypeId == GeometryTypeId.GEOS_LINEARRING:
        ...
        elif geom.geometryTypeId == GeometryTypeId.GEOS_MULTIPOINT:
            return self.transformMultiPoint(geom, None)
        elif geom.geometryTypeId == GeometryTypeId.GEOS_MULTILINESTRING:
            return self.transformMultiLineString(geom, None)
        elif geom.geometryTypeId == GeometryTypeId.GEOS_MULTIPOLYGON:
            return self.transformMultiPolygon(geom, None)
        elif geom.geometryTypeId == GeometryTypeId.GEOS_GEOMETRYCOLLECTION:
    ...
    def transformMultiPoint(self, geom, parent):
        ...
        return self._factory.buildGeometry(transGeom)
    ...
    def transformLineString(self, geom, parent):
        seq = self.transformCoordinates(geom.coords, geom)
        self._factory.createLineString(seq)
    ...
    def transformPolygon(self, geom, parent):
        ...
        if exterior is None or exterior.geometryTypeId != GeometryTypeId.GEOS_LINEARRING or exterior.is_empty:
    ...
    def transformGeometryCollection(self, geom, parent):
        ...
        if self.preserveGeometryCollectionType:
            return self._factory.createGeometryCollection(transGeom)
        else:
            return self._factory.buildGeometry(transGeom)
...
class Envelope():
    ...
    def __str__(self) -> str:
        return "Env[{}:{},{}:{}]".format(self.minx, self.maxx, self.miny, self.maxy)
    ...
    * \todo Suggestion: add equal and not-equal operator to this class.
    ...
    def matches(self, required: str) -> bool:
        ...
        for i in range(self.firstDim):
            for j in range(self.secondDim):
                if not IntersectionMatrix.static_matches(_matrix[i][j], required[3 * i + j]):
                    return False
        return True
    ...
    def isTouches(self, dimensionOfGeometryA: int, dimensionOfGeometryB: int) -> bool:
        ...
        if ((dimensionOfGeometryA == Dimension.A and dimensionOfGeometryB == Dimension.A) or
            ...
            return self.matrix[Location.INTERIOR][Location.INTERIOR] == Dimension.FALSE and (
                self.matches(self.matrix[Location.INTERIOR][Location.BOUNDARY], 'T') or
                self.matches(self.matrix[Location.BOUNDARY][Location.INTERIOR], 'T') or
                self.matches(self.matrix[Location.BOUNDARY][Location.BOUNDARY], 'T')
                )
    ...
    def isCrosses(self, dimensionOfGeometryA: int, dimensionOfGeometryB: int) -> bool:
        ...
        if ((dimensionOfGeometryA == Dimension.P and dimensionOfGeometryB == Dimension.L) or
            ...
            return (self.matches(self.matrix[Location.INTERIOR][Location.INTERIOR], 'T') and
                    self.matches(self.matrix[Location.INTERIOR][Location.EXTERIOR], 'T'))
        ...
        if ((dimensionOfGeometryA == Dimension.L and dimensionOfGeometryB == Dimension.P) or
            ...
            return (self.matches(self.matrix[Location.INTERIOR][Location.INTERIOR], 'T') and
                    self.matches(self.matrix[Location.EXTERIOR][Location.INTERIOR], 'T'))
    ...
    def isOverlaps(self, dimensionOfGeometryA: int, dimensionOfGeometryB: int) -> bool:
        ...
        if ((dimensionOfGeometryA == Dimension.P and dimensionOfGeometryB == Dimension.A) or
            ...
            return (self.matches(self.matrix[Location.INTERIOR][Location.INTERIOR], 'T') and
                self.matches(self.matrix[Location.INTERIOR][Location.EXTERIOR], 'T') and
                self.matches(self.matrix[Location.EXTERIOR][Location.INTERIOR], 'T'))
    ...
    def isCovers(self) -> bool:
         ...
         * second
        """
        return ((self.matches(self.matrix[Location.INTERIOR][Location.INTERIOR], 'T') or
                self.matches(self.matrix[Location.INTERIOR][Location.BOUNDARY], 'T') or
                self.matches(self.matrix[Location.BOUNDARY][Location.INTERIOR], 'T') or
                self.matches(self.matrix[Location.BOUNDARY][Location.BOUNDARY], 'T')) and
            self.matrix[Location.EXTERIOR][Location.INTERIOR] == Dimension.FALSE and
            self.matrix[Location.EXTERIOR][Location.BOUNDARY] == Dimension.FALSE)
...
class CoordinateSequence(list):
    ...
    def __init__(self, coords=None):
        ...
        if coords is not None:
            self.add(coords, allowRepeated=True, direction=True)
    ...
    def clone(self):
        return CoordinateSequence([co for co in self])
...
class LinearComponentExtracter():
    ...
    def filter_rw(self, geom) -> None:
        if (geom.geometryTypeId == GeometryTypeId.GEOS_LINESTRING or
                geom.geometryTypeId == GeometryTypeId.GEOS_LINEARRING):
...
class PolygonExtracter():
    ...
    def getPolygons(self, geom, ret) -> None:
...
class ComponentCoordinateExtracter(GeometryComponentFilter):
    ...
    @staticmethod
    def getCoordinates(geom, ret: list) -> None:
        """
        ...
        """
        cce = ComponentCoordinateExtracter(ret)
        geom.apply_ro(cce)

    def filter_rw(self, geom) -> None:
        if (geom.geometryTypeId == GeometryTypeId.GEOS_LINEARRING or
               geom.geometryTypeId == GeometryTypeId.GEOS_LINESTRING or
               geom.geometryTypeId == GeometryTypeId.GEOS_POINT):
