...
"""
XML_PATH = """<path id="{}" style="fill:{};stroke:{};stroke-width:{}px;stroke-linecap:butt;stroke-linejoin:round;stroke-opacity:1" d="{}"/>
"""
XML_TEXT = """<text id="{}" style="font-style:normal;font-weight:normal;font-size:{}px;line-height:125%;font-family:sans-serif;letter-spacing:0px;word-spacing:0px;fill:{};fill-opacity:1;stroke:none;stroke-width:1px;stroke-linecap:butt;stroke-linejoin:miter;stroke-opacity:1;text-anchor:middle;text-align:center;" x="0" y="0" transform="{}">{}</text>
"""
XML_GROUP = """<g id="{}">
"""
XML_GROUP_END = """</g>
...
def SVG_blenderCurve(itM, curve, line_width, fill_color, stroke_color, components, as_group=True):
    """
    ...
    if as_group:
        for i, spline in enumerate(curve.data.splines):
            components.append(SvgPath("{}-{}".format(name, i), tM, spline, line_width, fill_color, stroke_color))
        return SvgGroup(name, components)
    else:
        for i, spline in enumerate(curve.data.splines):
            if i == 0:
                svg_path = SvgPath("{}-0".format(name), tM, spline, line_width, fill_color, stroke_color)
            else:
                svg_path.add_shape(tM, spline, fill_color)
        components.append(svg_path)
        return svg_path
        
        
def SVG_dimension(itM, dimension, scale, line_width, dim_color):
    components = []
    for txt in dimension.children:
        components.append(SvgText(itM * txt.matrix_world, txt, scale, dim_color))
    # add the curve into components
    SVG_blenderCurve(itM, dimension, line_width, dim_color, dim_color, components, as_group=False)
    return SvgGroup("{}".format(dimension.name), components)
 
 
def SVG_wall_childs(context, itM, wall, scale, line_width, window_color, stroke_color, dim_width, dim_color, openings, dimensions):
    ...
    for child in wd.childs:
        ...
        if d is None:
            if "archipack_wall2" in c.data:
                SVG_wall_childs(context, itM, c, scale, line_width, window_color, stroke_color, dim_width, dim_color, openings, dimensions)
        else:
            symbol = d.as_2d(context, c)
            # add window in her own group, let separate curves so panels fill override frame
            openings.append(SVG_blenderCurve(itM, symbol, 0.2 * line_width, window_color, stroke_color, [], as_group=True))
            remove_curve(context, symbol)
            # window / door dimensions
            for child in c.children:
                d = child.data
                if d:
                    if "archipack_dimension" in d:
                        dimensions.append(SVG_dimension(itM, child, scale, dim_width, dim_color))
    # wall dimensions
...
def SVG_wall(context, itM, wall, scale, line_width, wall_color, hole_color, window_color, stroke_color, dim_width, dim_color):
    """
    ...
    # wall plain parts
    SVG_blenderCurve(itM, w, line_width, wall_color, stroke_color, parts, as_group=False)
    remove_curve(context, w)
          
    # wall fill under windows
    if len(f) > 0:
        f = f[0]
        SVG_blenderCurve(itM, f, line_width, hole_color, stroke_color, parts, as_group=False)
        remove_curve(context, f)
        
    # windows / doors
    SVG_wall_childs(context, itM, wall, scale, line_width, window_color, stroke_color, dim_width, dim_color, openings, dimensions)
...
class ARCHIPACK_OP_ExportSvg(Operator, ExportHelper):
    ...
    def execute(self, context):
        ...
        curves = [o for o in context.scene.objects if o.type in {'CURVE', 'FONT'}]
        ...
        # 0, 0 is bottom left
        origin = layout.matrix_world.translation
                ...
                if child.data and "archipack_dimension" in child.data:
                    ...
                    for c in child.children:
                        if c.name in curves:
                            del curves[c.name]
            # window / door dimensions
            for child in wall.data.archipack_wall2[0].childs:
                c, d = child.get_child(context)
                if d is not None:
                    for child in c.children:
                        if child.data and "archipack_dimension" in child.data:
        ...
        svg_walls = [
            SVG_wall(
                context, itM, w, s, 
                line_width, 
                wall_color, hole_color, window_color, 
                stroke_color, 
                dim_width, dim_color
        ...
        for curve in curves.values():
            ...
            if curve.type == 'CURVE':
                svg_lines.append(SVG_blenderCurve(itM, curve, line_width, fill_color, dim_color, []))
                
            elif curve.type == 'FONT':
                # Export text
                svg_lines.append(SvgText(tM, curve, s, dim_color))
        ...
        for stair in stairs:
            ...
            svg_lines.append(SVG_blenderCurve(itM, curve, line_width, fill_color, stroke_color, [], as_group=False))
